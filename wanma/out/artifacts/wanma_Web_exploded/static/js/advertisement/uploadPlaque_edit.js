	...
	$("#editPeImageList")
			.prepend(
					'<t><img  src="'
							+ url
							+ '" style="width: 150px; height: 150px;" />'
							+ '<a  id="Pdelete_edit" href="javascript:void(0);" onclick="deleteFile_list(\''
							+ url + '\',this,\'' + dataId + '\')">删除</a></t>');
}
...
function FileUploadStart(file) {
	...
	errorFlag = true;
	var fileSize = $("#editPeImage t").length;
	if (fileSize >= 1) {
...
function splashUploadStart_list(file) {
	errorFlag = true;
	var fileSize = $("#editPeImageList t").length;
	if (fileSize >= 1) {
		this.cancelUpload(file.id);
		$('#' + file.id).remove();
		alertMsg.error("只能发最多1个文件");
...
function uploadify_onSelectError(file, errorCode, errorMsg) {
	...
	case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
		// this.queueData.errorMsg = "每次最多上传 " + this.settings.queueSizeLimit +
		// "个文件";
