...
function deleteFile_list(imageUrl,obj,referenceId){
	...
	function FileUploadSuccess(file, data, response){
		...
		$("#advPicFileId").val($("#advPicFileId").val()+dataId);
		var url=obj.navTabId;
		$("#editShImage").prepend('<t><img  src="'+url+'" style="width: 150px; height: 150px;" />'
	...
	function splashUploadSuccess_list(file, data, response){
		var obj=JSON.parse(data);
		var dataId = obj.message;
		$("#advPicFileId_list").val($("#advPicFileId_list").val()+dataId);
		var url=obj.navTabId;
		$("#editShImageList").prepend('<t><img  src="'+url+'" style="width: 150px; height: 150px;" />'
				+'<a  id="delete_edit" href="javascript:void(0);" onclick="deleteFile_list(\''+url+'\',this,\''+dataId+'\')">删除</a></t>');
	}
	...
	function FileUploadStart(file) {
		...
		errorFlag=true;
	    var fileSize=$("#editShImage t").length;
	    if(fileSize>=1){ 
	...
	function splashUploadStart_list(file) {
		errorFlag=true;
	    var fileSize=$("#editShImageList t").length;
	    if(fileSize>=1){ 
	      this.cancelUpload(file.id); 
	      $('#' + file.id).remove();
	      alertMsg.error("只能发最多1个文件");
