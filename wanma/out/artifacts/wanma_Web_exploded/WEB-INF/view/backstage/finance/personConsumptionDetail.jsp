...
<form id="pagerForm" method="post" action="finance/personConsumeDetail.do" onsubmit="return navTabSearch(this);"> 
			...
			<tbody>
				...
				<!-- value="搜索关键字"  -->
		          ...
		          <tr>
		            ...
		            <td style="align:left">
						<span>城市&nbsp;</span>
						<select class="provinceCode required"  id="provinceCodef10" name="provinceCode" next="cityCodef10"  style="float: none;width:130px;">
						...
						<select id="cityCodef10" data-val="${params.cityCode}" name="cityCode" class="required" style="float: none;width:130px;">
					...
					<td align="right">
   						<bmtag:button messageKey="excel导出" type="button" onclick="exportSubmit('pagerForm','financeExcelReport/getPersonConsumeDetailData.do')"/>
					</td>
				</tr>
			 </tbody>
		</table>
	...
	<table class="table" width="100%" layoutH="140">
		<thead>
			<tr align="center">
				...
				<th><bmtag:message messageKey="充电电量(度数)" /></th>
		...
		<c:forEach items="${list}" var="obj" varStatus="status">
			<tr target="objId" align="center">
				...
				<td>${obj['消费金额(元)']}</td>
				<td>${obj['实际优惠金额(元)']}</td>
				<td><fmt:formatDate value="${obj['消费时间']}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td>
			    	<c:choose>
						<c:when test="${obj['充电电量'] == ''}">0.00</c:when>
