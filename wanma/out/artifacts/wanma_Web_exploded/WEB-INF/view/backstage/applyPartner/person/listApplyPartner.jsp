...
<script type="text/javascript">
	...
	function bohui2(obj){
		...
		navTab.openTab("applyPersonPartner", basepath+"/admin/applyPartner/changePersonApplyReason.do", { title:"驳回&编辑原因", fresh:false, data:{applyPartnerId:ids}});
...
<form id="pagerForm" method="post" action="applyPartner/personApplyList.do" onsubmit="return navTabSearch(this);"> 
		...
		<table class="searchContent" >
			<tbody>
				<tr >		            
					<td>
			            <span>提交时间&nbsp;</span>			             
			              <input id="applyPersonDateStarte" name="createDateStarte" value="${tblApplyPartner.createDateStarte }" class="date"  style="width:155px"
						 onClick="WdatePicker({el:'applyPersonDateStarte',minDate:'2015-01-01',maxDate:'#F{$dp.$D(\'applyPersonDateEnd\')}'})" >
			                                    至 <input id="applyPersonDateEnd" name="createDateEnd" value="${tblApplyPartner.createDateEnd }" class="date"  style="width:155px"
			             onClick="WdatePicker({el:'applyPersonDateEnd',minDate:'#F{$dp.$D(\'applyPersonDateStarte\')}'})">                        	                                    
		            </td>					
				</tr>
				<tr >
<%-- 				<td>
...
<div class="pageContent">
	...
	<%-- 		<c:if test="${loginUser.userLevel!='1'}"> --%>
				<li><bmtag:link href="applyPartner/checkPersonPartnerDetail.do?applyPartnerId={pkApplyPartner}"
						target="navTab" rel="查看详情"
						messageKey="查看详情" dwzClass="add" /></li>
				<%-- <li><bmtag:link href="applyPartner/changePersonApplyReason.do?applyPartnerId={pkApplyPartner}"
						target="navTab" rel="编辑驳回原因"
						messageKey="驳回&编辑原因" dwzClass="edit" /></li> --%>
						
				<li><a class="icon"  rel="ids"
					postType="string"  onclick="bohui2(this)"
					title="驳回&编辑原因"><span>驳回&编辑原因 </span>
				</a></li>
			<%-- </c:if> --%>
			<li><bmtag:link
						href="applyPartner/deletePersonApplyById.do?pageNum=${pager.pageNum }&applyPartnerId={pkApplyPartner}"
				...
				<li><a class="delete" target="selectedTodo" rel="ids"
					...
					href="<c:url value='/admin/applyPartner/deletePersonApply.do'/>"
	...
	<table class="table" width="100%" layoutH="136">
		...
		<c:forEach items="${applyPartnerList}" var="applyPa" varStatus="status">
			<tr target="pkApplyPartner" rel="${applyPa.pkApplyPartner }" align="center">
				...
				<td>${applyPa.apPaStationAddr}</td>
				<td>
				<c:if test="${applyPa.apPaStationType == 1}">家庭充电点</c:if>
				<c:if test="${applyPa.apPaStationType == 2}">目的地充电点</c:if>
				<c:if test="${applyPa.apPaStationType == 3}">其他(备注:${applyPa.apPaStationTypeText})</c:if>
				</td>				
				<td>
					<fmt:formatDate value="${applyPa.apPaCreatedate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
				<c:if test="${applyPa.apPaInstallation == '1'}">是</c:if>
				<c:if test="${applyPa.apPaInstallation == '2'}">否</c:if>
				</td>			
				<td>
					<c:choose>
				...
				<td>
					<c:choose>
						<c:when test="${applyPa.apPaProcessState == '1'}">
							<a target="ajaxTodo" href="<c:url value='/admin/applyPartner/updatePersonApplyState.do?pkApplyPartner=${applyPa.pkApplyPartner}&apPaProcessState=2'/>" title="确定通过该申请？" style='color:#0099FF'>通过申请</a>
						</c:when>
						<c:when test="${applyPa.apPaProcessState == '2'}">
							<a target="ajaxTodo" href="<c:url value='/admin/applyPartner/updatePersonApplyState.do?pkApplyPartner=${applyPa.pkApplyPartner}&apPaProcessState=3'/>"  style='color:#0099FF'>完成信息审核</a>
						</c:when>
						<c:when test="${applyPa.apPaProcessState == '3'}">
							<a target="ajaxTodo" href="<c:url value='/admin/applyPartner/updatePersonApplyState.do?pkApplyPartner=${applyPa.pkApplyPartner}&apPaProcessState=4'/>"  style='color:#0099FF'>完成合作洽谈</a>
						</c:when>
						<c:when test="${applyPa.apPaProcessState == '4'}">
							完成加入
						</c:when>
