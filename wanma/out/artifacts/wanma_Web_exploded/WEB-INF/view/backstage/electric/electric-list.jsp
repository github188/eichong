			...
			<table class="searchContent">
				<tbody>
					<tr>
						...
						<td><label><bmtag:message
									messageKey="连接状态" /> </label>
						</td>
						<td><select name="comm_status" class="select_Style">
								<option value="">全部</option>
								<option value="1" ${tblElectricpile.comm_status=="1"
									? 'selected="selected"' : ''}>已连接</option>
								<option value="0" ${tblElectricpile.comm_status=="0"
					...
					<tr>
						...
						<td><select name="elpiState" class="select_Style">
								...
								<option value="12" ${tblElectricpile.elpiState==12
					...
					<tr>
						...
						<td><select name=elpiPowerinterface class="select_Style">
								...
								<c:forEach var="item" items="${param5}">
									<option value="${item.key}"
										${item.key == tblElectricpile.elpiPowerinterface ? 'selected="selected"' : ''} >
										${item.value.coCo_Content}</option>
							     </c:forEach>
						</select>
						</td>
						...
						<c:choose>
							<c:when
								...
								<td><label><bmtag:message
											messageKey="所有权" /> </label>
								</td>
								<td><input name="ownerShip"
									value="${tblElectricpile.ownerShip}" />
								</td>
					</tr>
					...
					<tr>
						...
						<td colspan="2">
							<select class="provinceCode required"  id="electricSelProvince" next="electricSelCity" name="elPiOwnProvinceCode"  style="float: none;width:130px;">
							...
							<select class="cityCode required" id="electricSelCity" next="electricSelDistrict" data-val="${tblElectricpile.elPiOwnCityCode}" name="elPiOwnCityCode"  style="float: none;width:130px;">
								 <option value="">--请选择城市--</option>
							</select>
							<select id="electricSelDistrict" data-val="${tblElectricpile.elPiOwnCountyCode}" name="elPiOwnCountyCode" class="required"  style="float: none;width:130px;">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li>
				<bmtag:link isAuth="true" target="navTab" href="electric/addElectricPileUi.do"
					rel="electricOperate" messageKey="common.icon.new" dwzClass="add" id="electricAddPage" />
			</li>
			<li>
				<bmtag:link isAuth="true" target="navTab" href="electric/changeElectricPileUi.do?id={id}"
					 rel="electricOperate" messageKey="common.icon.edit" dwzClass="edit" />
			</li>
			<li>
				<bmtag:link isAuth="true" target="selectedTodo" href="electric/removeElectricPile.do"
					rel="ids" postType="string" altKey="确定删除吗？" dwzClass="delete" messageKey="批量删除" />
			</li>
			<li>
				<bmtag:link isAuth="true" target="navTab" href="electric/showElectricPile.do?id={id}"
					 rel="electricShowPage" messageKey="common.icon.view" dwzClass="icon" id="electricShowPage" />
			</li>
			<li>
				<bmtag:link isAuth="true"  target="selectedTodo" href="electric/changeElectricPileState.do?changeType=examinePage"
			 		rel="ids" postType="string" altKey="确定提审吗？" dwzClass="edit" messageKey="提审" />
			</li>
			<li>
				<bmtag:link isAuth="true"  target="selectedTodo" href="electric/changeElectricPileState.do?&changeType=offLinePage"
					 rel="ids" postType="string" altKey="确定要申请专属吗？" dwzClass="edit" messageKey="专属申请" />
			</li>
			<!--  <li>
			...
			<li>
				<bmtag:link isAuth="true"  target="selectedTodo" href="electric/shareApplication.do?&changeType=toApplication"
					 rel="ids" postType="string" altKey="确定要分享申请吗？" dwzClass="edit" messageKey="分享申请" />
			</li>
			<li>
				<bmtag:link isAuth="true" target="navTab" href="electric/toImport.do"
					rel="electricOperate" messageKey="导入" dwzClass="edit"  />
			</li>
		</ul>
	</div>
	<table class="table" width="100%" layoutH="200">
		<thead>
			<tr align="center">
				...
				<th><bmtag:message messageKey="运营平台" />
				</th>				
				...
				<th><bmtag:message messageKey="所属区域" /><!--bmtag:message messageKey="所属区/县" /-->
				</th>
		...
		<tbody>
			<c:forEach items="${electricList}" var="electric" varStatus="status">
				<tr target="id" rel="${electric.pk_ElectricPile}" align="center">
					<td><input name="ids" value="${electric.pk_ElectricPile}" data-value="${electric.elPi_ElectricPileCode}:${electric.elPi_RateInformationId}:${electric.elPi_State}:${electric.comm_status}"
						type="checkbox">
					</td>
					...
					<td>
						<c:if test="${electric.elPi_OwnerCompany ==0}">其他</c:if> 
						<c:if test="${electric.elPi_OwnerCompany ==1}">爱充</c:if> 
						<c:if test="${electric.elPi_OwnerCompany ==2}">国网 </c:if> 
						<c:if test="${electric.elPi_OwnerCompany ==3}">特斯拉 </c:if>
					</td>
					<%-- <td>${electric.headSateDetail}</td>
					<td></td> --%>
					<td title="${electric.elPi_RejectionReason}">
						<c:if test="${electric.elPi_RejectionReason.length()>10}">
					...
					<td>
						<c:if test="${electric.comm_status == '0'}"><p class="pCenterRed">未连接</p></c:if>
						<c:if test="${electric.comm_status == '1'}"><p class="pCenterRedGreen">已连接</c:if>
					</td>
					<td>
						<c:if test="${electric.elPi_Binding == '0'}"><p class="pCenterRed">否</p></c:if>
						<c:if test="${electric.elPi_Binding == '1'}"><p class="pCenterRedGreen">是</p></c:if>
					</td>
					
					<td>
						<c:if test="${electric.elPi_IsAppoint == '0'}"><p class="pCenterRed">否</p></c:if>
						<c:if test="${electric.elPi_IsAppoint == '1'}"><p class="pCenterRedGreen">是</p></c:if>
					</td>
