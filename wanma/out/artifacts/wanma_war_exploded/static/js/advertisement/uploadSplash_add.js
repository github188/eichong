...
function deleteFile(imageUrl,obj,referenceId){
		...
		var businessType=$("#businessType_add").val();
		$.ajax({
			type : 'post',
...
function deleteFile_list(imageUrl,obj,referenceId){
	var fileName=imageUrl.substring(imageUrl.lastIndexOf("/")+1,imageUrl.length);
	var businessType=$("#splashListType_add").val();
	$.ajax({
		type : 'post',
		url : basepath+"/common/deleteFile.do",
		dataType : "json",
	...
	function FileUploadSuccess(file, data, response){
		...
		$("#advPicFileId_add").val($("#advPicFileId_add").val()+dataId);
		var url=obj.navTabId;
		$("#addShImage").prepend('<t><img  src="'+url+'" style="width: 150px; height: 150px;" />'
	...
	function splashUploadSuccess_list(file, data, response){
		var obj=JSON.parse(data);
		var dataId = obj.message;
		$("#advListFileId_list_add").val($("#advListFileId_list_add").val()+dataId);
		var url=obj.navTabId;
		$("#addShImageList").prepend('<t><img  src="'+url+'" style="width: 150px; height: 150px;" />'
	...
	function FileUploadStart(file) {
		...
		errorFlag=true;
	    var fileSize=$("#addShImage t").length;
	    if(fileSize>=1){ 
	...
	function splashUploadStart_list(file) {
		errorFlag=true;
	    var fileSize=$("#addShImageList t").length;
	    if(fileSize>=1){ 
	      this.cancelUpload(file.id); 
	      $('#' + file.id).remove();
	      alertMsg.error("只能发最多1个文件");
