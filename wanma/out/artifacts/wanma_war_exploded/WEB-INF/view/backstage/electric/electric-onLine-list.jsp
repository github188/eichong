...
<script type="text/javascript">
	...
	function checkSelect(obj){
		...
		if (ids.length==0) {
			...
			navTab.openTab("", basepath+"/admin/electric/onlineReasonUi.do", { title:"驳回", fresh:false, data:{id:ids.join()}});
		}
...
<div class="pageHeader">
	<form id="pagerForm" method="post" action="electric/getOnLineElectricPileList.do"
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
		 <li>
		 	<bmtag:link isAuth="true"  target="selectedTodo" href="electric/changeElectricPileState.do?changeType=onLinePage"
		...
		<tbody>
			<c:forEach items="${electricList}" var="electric" varStatus="status">
				<tr target="id" rel="${electric.pk_ElectricPile}" align="center">
				    <td><input name="ids" value="${electric.pk_ElectricPile}" type="checkbox"></td>
					...
					<td>${electric.elPi_ElectricPileName}</td>
					 ...
					 <td><c:if test="${electric.elPi_State ==0}" >草稿</c:if>
					    ...
					    <c:if test="${electric.elPi_State ==3}" >已驳回 </c:if>
					...
					<%-- <c:if test="${electric.elPi_Longitude!=null}" >经度：${electric.elPi_Longitude}</c:if><c:if test="${electric.elPi_Latitude!=null}" >&nbsp;&nbsp;纬度：${electric.elPi_Latitude}</c:if> --%>
					</td>
					<td>
