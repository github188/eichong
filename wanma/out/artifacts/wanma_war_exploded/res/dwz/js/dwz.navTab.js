...
var navTab = {
	...
	_stopIntervalF: function(){
		var intervalIds = navTab.getCurrentPanel().find("#intervalIds").val();
		if(intervalIds){
			var intervalIdArray = intervalIds.split(",");
			if(intervalIdArray != null && intervalIdArray.length > 0){
				for(var i=0;i<intervalIdArray.length;i++){
					var intervalIdTemp = eval(intervalIdArray[i]);
					if(intervalIdTemp){
	...
	_selectedTabReloadF: function(){
		var thisF = navTab.getCurrentPanel().find("#reloadF");
		var reloadFs = thisF.val();
		if(reloadFs){
			var reloadFArray = reloadFs.split(',');
			if(reloadFArray != null && reloadFArray.length > 0){
				var param = thisF.attr('data-param');
				for(var i=0;i<reloadFArray.length;i++){
					var reloadf = reloadFArray[i];
					if(param){
						var paramFIndex = parseInt(thisF.attr('paramIndex'));
						if(i==paramFIndex){
							eval(reloadf+"\("+param+"\)");
						}
					}else{
	...
	_switchTab: function(iTabIndex){
		this._stopIntervalF();//取消当前页定时刷新脚本
		...
		this._scrollCurrent();
		this._selectedTabReloadF();//重新执行被选择页签的定时脚本
		this._reload($tab);
	},
