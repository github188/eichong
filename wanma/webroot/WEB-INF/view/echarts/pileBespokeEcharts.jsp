...
<form id="pagerForm" method="post" action=""> 
	...
	<div id="myChartPileBespokeCountDiv"
		style="height:500px; border: 1px solid #ccc; padding: 10px;"></div>

</form>
</body>
<script type="text/javascript">
	var myChartPileBespokeCountDiv;
	var begin_date_val;
	var end_date_val;
	require.config({
		paths : {
	...
	function drowecharts() {
		...
		require([ 
			...
			myChartPileBespokeCountDiv = ec.init(document
					.getElementById('myChartPileBespokeCountDiv'), theme);
			initChart()
		});
	}

	function initChart() {
		if (myChartPileBespokeCountDiv) {
			myChartPileBespokeCountDiv.clear();
		}
		...
		$.ajax({
			...
			success : function(json) {
				myChartPileBespokeCountDiv.setOption(json);
				$.ajax({
					type : 'POST',
					url : ajaxUrl,
					dataType : "json",
					...
					success : function(json) {
						if(json.data1 != null){
							myChartPileBespokeCountDiv.setOption(json.data1);		
						}		
					}
				});
			}
