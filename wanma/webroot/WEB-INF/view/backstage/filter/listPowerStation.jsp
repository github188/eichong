		...
		<input type="hidden" name="status" value="${pager.status}" /> <input
	 	...
	 	<div class="searchBar">
			<table class="searchContent">
				...
				<td style="align: left"><span>站点状态</span> <select
							name="powerState"  class="select_Style"
							style="width: 150px;">
								<option value="">全部</option>
								<option value="10"
									${TblPowerstation.powerState=="10"
									? 'selected' : ''}>专属</option>
								<option value="15"
									${TblPowerstation.powerState=="15"
									? 'selected' : ''}>分享</option>
						...
						<td align="right"><bmtag:button messageKey="excel导出"
								type="button"
								onclick="exportSubmit('pagerForm','filter/filterExport.do')" />
						</td>
	...
	<table class="table" width="100%" layoutH="132">
		<thead>
			...
			<th><input type="checkbox" group="ids" class="checkboxCtrl">
				...
				<th><bmtag:message messageKey="站点状态" /></th>
		...
		<tbody>
			<c:forEach items="${powerList}" var="item" varStatus="status">
				<tr target="id" rel="${item.pkPowerstation}" align="center">
					...
					<td>
					<c:if test="${item.powerState ==10}"><a style="color:blue;">专属</a></c:if> 
					<c:if test="${item.powerState ==15}"><a style="color:red;">分享</a></c:if></td>
					<td>${item.cityName}</td>
