...
<form id="pagerForm" method="post" action="finance/businessChargeDetail.do" onsubmit="return navTabSearch(this);"> 
		...
		<table class="searchContent">
			<tbody>
				...
				<!-- value="搜索关键字"  -->
					...
					<td>
			            <span>充电时间&nbsp;</span>				             	 
			             <input id="startDate_f3" name="startDate" value="${params.startDate}" class="date"  style="width:155px"
						 onClick="WdatePicker({el:'startDate_f3',minDate:'2015-01-01',maxDate:'#F{$dp.$D(\'endDate_f3\')}'})">
			                                    至 <input id="endDate_f3" name="endDate" value="${params.endDate}" class="date"  style="width:155px"
			             onClick="WdatePicker({el:'endDate_f3',minDate:'#F{$dp.$D(\'startDate_f3\')}'})">                       	            
		            </td>
		          </tr>
		          <tr>
		            <td style="align:left">
						<span>城市&nbsp;</span>
						<select class="provinceCode required"  id="provinceCodef3" name="provinceCode" next="cityCodef3"  style="float: none;width:130px;">
						...
						<select id="cityCodef3" data-val="${params.cityCode}" name="cityCode" class="required" style="float: none;width:130px;">
			  ...
			  <td style="align:left">
					...
					<td style="align:right">
						<bmtag:button messageKey="excel导出" type="button" onclick="exportSubmit('pagerForm','financeExcelReport/getBusinessChargeDetailData.do')"/>
					</td>
				</tr>
			 </tbody>
		</table>
...
<div class="panelBar">
		<ul class="toolBar">
			<li>
				<bmtag:link isAuth="true"  target="selectedTodo" href="finance/orderAccounts.do?type=businessChargeDetail"
			 		rel="ids"  postType="string" altKey="确定结算吗？" dwzClass="edit" messageKey="结算" />
			</li>
		</ul>
	</div>
	<!--浮动的尖峰平谷汇率-->
	<div id="JFPG_BOX_DIV_BusinessP"
		style="height: auto; position: absolute; right: 20px; display: none">
		<ul id="JFPG_BOX_DIV_Business" class="JFPG_BOX">
		</ul>
		<span class="closeP" onclick="closeBusinessChargeWindow();">X</span>
	</div>
	<table class="table" width="100%" layoutH="137">
		<thead>
			...
			<th><input type="checkbox" group="ids" class="checkboxCtrl">
				...
				<th><bmtag:message messageKey="充电金额(元)" /></th>
