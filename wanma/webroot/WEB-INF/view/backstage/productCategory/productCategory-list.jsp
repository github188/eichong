...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><bmtag:link href="productCategory/newCategory.do"
					 actionId="ACT-009-002" target="navTab"
					rel="newCategory" messageKey="添加" dwzClass="add" /></li>
			<li><bmtag:link href="productCategory/editCategory.do?id={id}"
					 actionId="ACT-009-003" target="navTab"
					rel="editCategory" messageKey="编辑" dwzClass="edit" /></li>
			<li><bmtag:link
					href="productCategory/deleteById.do?pageNum=${pager.pageNum }&id={id}"
			...
			<li><a class="delete" target="selectedTodo" rel="ids"
				...
				href="<c:url value='/admin/productCategory/deleteCategorys.do'/>"
	...
	<table class="table" width="100%" layoutH="87">
		<thead>
			<tr align="center">
				...
				<th><bmtag:message messageKey="分类名称" /></th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${categoryList}" var="category" varStatus="status">
				<tr target="id" rel="${category.pkProductcategory }" align="center">
					<td><input name="ids"  value="${category.pkProductcategory}"
						type="checkbox" />
					</td>
					<td>${ status.index + 1 + pager.numPerPage*(pager.pageNum-1)}</td>
					<td>${category.prcaName}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
