	...
	<table class="table" width="100%" layoutH="190">
		...
		<c:forEach items="${userMoneyRecordList}" var="userMoneyRecord" varStatus="status">
			<tr target="puHi_UserId" rel="${userMoneyRecord.puHi_UserId }" align="center">			
				...
				<td><a target="navTab"  href="<c:url value='/admin/moneyRecord/getUserMoneyRecordDetials.do?puHi_UserId=${userMoneyRecord.user_id }'/>" style='color:#0099FF'>${userMoneyRecord.user_account}</a></td>
				...
				<td>${userMoneyRecord.userTotal_Coupon }</td>
				<td>${userMoneyRecord.norm_account }</td>

				<td>
				<c:if test="${!(userMoneyRecord.norm_account >0) }"><a style='color:#757575'>退款</a></c:if>
				<c:if test="${userMoneyRecord.norm_account >0 }"><a target="navTab"  href="<c:url value='/admin/moneyRecord/getRecordRefund.do?userId=${userMoneyRecord.user_id }&user_account=${userMoneyRecord.user_account}'/>" style='color:#0099FF'>退款</a></c:if>
				</td>
			</tr>
		</c:forEach>
		</tbody>
