...
<body>
 <jsp:include page="../common/header2.jsp" />
	<!--//header -->
<input id="powerStationId" tip-value="2" class="tip"  type="hidden" name="prCoProductId" value="${station.powerStationId}"> 
<input id ="zhanxingpingshu" type="hidden" value ="${station.powerCommentStar}">
...
<div class="detail">
  ...
  <div class="ChannelContent">
    <dl class="detailHeader">
      <dt style="position: relative;"><!--单一桩体，非站点<span class="biaoshi_zhuang"></span>--><span class="biaoshi_zhan"></span>${station.powerStationName}<span id='fenxiangDiv' class="fenxiang"><a href="#" id='shareOpenDiv'></a><div id='shareTxtDiv'>分享</div>
     </span>
     <div id='bshare-custom_div' data-info="${station.postLongitude},${station.postLatitude},1,${station.powerStationAddress}" class="bshare-custom1" style="display:none"><a title="分享到QQ空间"  class="bshare-qzone" ></a><a title="分享到新浪微博" class="bshare-sinaminiblog"></a><a title="分享到微信" class="bshare-weixin"></a></div>
      <span class="shoucang"><a class="collect_submit" data-id="${station.powerStationId}" data-type="2" data-value="${station.isCollect}"></a><div id='shouchangDiv'><c:if test="${station.isCollect=='0'}">收藏</c:if><c:if test="${station.isCollect != '0'}">已收藏</c:if></div></span></dt>
        ...
        <dd class="zhuangUp">
          ...
          <div class="zhuangC">
            <dl class="Sequence">
              ...
              <dd class="fontS48">0${pile.electricNum}<span>#</span></dd>
            ...
            <dl class="zhuangSide">
	          <c:forEach var="head" items="${pile.pileHeadList}" varStatus="status"  >
	          		<c:if test="${pile.electricPileConnStatus=='0'}">
						<c:if test="${head.pileHeadState==null}"><dd><span><strong></strong></span></dd></c:if>
  ...
  <div class="ChannelContent pad_bot50">
    <dl class="imgShow">
      <dt>图片展示</dt>
      <dd><img src="${station.powerStationImage}" width="300" height="200" alt="  " /></dd>
      <dd></dd>
      <dd></dd>
      <div style="clear:both;"></div>  
    </dl>
    ...
    <dl class="pingfen">
      <dt>评分</dt>
      <dd id="function-demo1" class="target-demo fen-main"></dd>
      <input id="zhanxingpingfenshu" tip-value=""  score-value=""  type="hidden" />  
      <dd><input name="button" type="button" id="zhanxingping-tijiao" value="提交" /></dd>
    </dl>
    <dl class="pinglun">
      ...
      <dd>
        <div class="BoxInput">
          <ul>
            ...
            <li class="user"><input id='zhanpingfen-tijiao' name="button" type="button" value="提交" /><span class="userNav"><img id="userImgDiv" class="userImg" src="${myImage}" width="27" height="27" alt="" style="display:none"/><t id='userNameDiv' class="userName">${userName}</t></span>
            </li>
          </ul>
        </div>
      </dd>
...
$(document).ready(function() {
	...
	$('body').on('mouseenter', '.showErweima', function (e) {
		...
		$("body").append(html);
		var psId=$("#powerStationId").val();
		$(".erweima").qrcode({
			width:'100',
			height:'100',
			correctLevel:0,
			text:"http://www.eichong.com/app/check_app_in.jsp?et=2&c=&d="+psId
		});	
	});
