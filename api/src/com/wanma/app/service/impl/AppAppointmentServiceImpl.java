...
import com.wanma.app.dao.AppAppointmentinstallationorderMapper;
import com.wanma.app.dao.AppInstalldetailMapper;
 ...
 */
@Service
public class AppAppointmentServiceImpl implements AppAppointmentService {
	/* 订单详情表操作dao */
	@Autowired
	...
	/* 预约安装订单表操作dao */
	@Autowired
	private AppAppointmentinstallationorderMapper appointmentMapper;
	/* 预约安装表操作dao */
	@Autowired
	private AppInstalldetailMapper installdetailMapper;
	/* 订单跟踪表操作dao */
	@Autowired
	...
	public void insertAppointment(TblAppointmentinstallationorder appointment,
			...
			String inDeQuantitys, String prodProductCode,String indeTotalamounts) {
		// 新增预约安装订单信息
		appointment.setAlorInstallationordercode(BluemobiCommon.getOrderNo());
		// 待安装
		appointment.setAlorInstallationorderstatus(1);
		...
		if (StringUtil.isNotEmpty(ordeProductids)) {
			...
			String[] totalamountArray = indeTotalamounts.split(",");
			...
			for (int i = 0; i < productidArray.length; i++) {
				...
				installdetail.setIndeTotalamount(new BigDecimal(totalamountArray[i]));
...
//		根据主键获取未提交的预约安装商品的总数和总价格(多个)
		...
		JSONObject.toJavaObject(JSONObject.parseObject(appointment), List.class);
		List<Map<String, Object>> list = JSONObject.toJavaObject(JSONObject.parseObject(appointment), List.class);;
		...
		appointmentOrder.setAlorTellogin(userinfoMapper.get(userId).getUsinPhone());
...
//			根据主键获取未提交的预约安装商品的总数和总价格(单个)
			...
			installdetail.setIndeInstallationaddress((String) map
					.get("pradAddress"));
			installdetail.setIndeInstallationperson((String) map.get("pradUsername"));
			installdetail.setIndeLnstallationtel((String) map.get("pradPhonenumber"));
