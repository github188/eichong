...
import com.base.common.ApplicationConsts;
...
import com.wanma.dao.AreaMapper;
...
public class ElectricPileListServiceImpl implements ElectricPileListService {
	...
	private static Logger log = Logger
			.getLogger(ElectricPileListServiceImpl.class);
	@Autowired
	...
	public void addElectricpile(TblElectricpile tblElectricpile,
		...
		if (userLevel == WanmaConstants.USER_LEVEL_BUSINESS)
			...
			tblElectricpile.setElPiUserName(GlobalSystem
					.getConfig("company.acw"));
		else {
			tblElectricpile.setElPiUserName(loginUser.getNormRealName());
			tblElectricpile.setCompanyNumber("0");
		}

		if (StringUtils.isBlank(tblElectricpile.getElpiTell())) {
			tblElectricpile.setElpiTell("4000850006");
		}
	...
	public void changeElectricpile(TblElectricpile tblElectricpile,
		...
		if (StringUtils.isEmpty(tblElectricpile.getOfflineTime())) {
	 ...
	 * 修改电桩状态，电桩ID
	 * 
	 * @param elpiElectricpileId
	 *            电桩编号
	 * @param elpiState
	 *            电桩状态
	 */
	@Override
	public String updateSateAndCode(String elpiElectricpileId, int elpiState) {
		TblElectricpile electricpile = new TblElectricpile();
		electricpile.setPkElectricpile(new Integer(elpiElectricpileId));
		electricpile = tblElectricpileMapper.get(electricpile);
		String elpiElectricpilecode=electricpile.getElpiElectricpilecode();
		electricpile = tblElectricpileMapper
				.getElectricPileInfo(elpiElectricpilecode);
		// 充电方式
		int ChargingMode = electricpile.getElpiChargingmode();
		if (ChargingMode==5) {
		...
		if (StringUtils.isEmpty(bigElectricpilecode)) {
			...
			Long a = new Long(bigElectricpilecode) + 1;
	...
	public void updateByElecId(TblElectricpile electric) {
		tblElectricpileMapper.updateByElecId(electric);
	}

	@Override
	public void updateStateByPowerIds(TblElectricpile electric) {
		tblElectricpileMapper.updateStateByPowerIds(electric);
	}
}
