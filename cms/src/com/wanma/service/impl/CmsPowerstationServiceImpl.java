...
public class CmsPowerstationServiceImpl implements CmsPowerstationService {
	...
	@Override
	public void removePowers(String ids) {
		 powerstationMapper.removePowers(ids);
		 commitLogService.insert("删除充电点，充电点主键：["+ids+"]");
	}
	/**
	...
	public void addPowers(TblPowerstation tblPowerstation,MultipartFile[]  listImage,MultipartFile[] detailImage,TblUser loginUser) {
		...
		if (userLevel == WanmaConstants.USER_LEVEL_BUSINESS)
			...
			tblPowerstation.setPoStUserName(GlobalSystem.getConfig("company.acw"));
		else
	...
	public TblPowerstation getPowerById(TblPowerstation tblPowerstation){
		...
		//获取电桩列表图片
			List<String> listImage=MultiFileUtil.getUrls(WanmaConstants.MULTI_TYPE_POWER_LIST_IMAGE,
					tblPowerstation.getPkPowerstation() + "");
	...
	@Override
	public void changeStateByIds(TblPowerstation tblPowerstation) {
		powerstationMapper.changeStateByIds(tblPowerstation);
		
	}

	@Override
	public TblPowerstation getPowerstationByName(TblPowerstation tblPowerstation) {
		return powerstationMapper.getPowerstationByName(tblPowerstation);
	}
}
