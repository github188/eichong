 ...
 */
@Controller
@RequestMapping("/manage/config/rateinfo")
public class RateInfoController extends BaseController{
	...
	* @date 2017-6-26
	...
	public void getRateInfoList(@ModelAttribute("pager") Pager pager,
		...
		if (rateInfoList == null) {
			rateInfoList = new ArrayList<RateInfoDO>();
		}
	...
	* @date 2017-6-26
	...
	@ResponseBody
	public void modifyRateInfo(@ModelAttribute RateInfoDO rateInfo, Model model) {
		//取当前登录用户
		UserDO user = getCurrentLoginUser();
		BaseResultDTO baseResultDTO = new BaseResultDTO();
		try {
	...
	* @date 2017-8-30
	 */
	@RequestMapping(value = "/getRateInfoById")
	@ResponseBody
	public void getRateInfoById(@ModelAttribute RateInfoDO rateInfo, HttpServletRequest request){
		JsonResult batchResult = new JsonResult();
		
		RateInfoDO result = rateInfoService.getRateInfoById(rateInfo);
		if (result == null) {
			result = new RateInfoDO();
		}
