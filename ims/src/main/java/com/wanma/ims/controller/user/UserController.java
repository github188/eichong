...
public class UserController extends BaseController {
    ...
    public void getUserList(UserDO searchModel, Pager pager) {
        ...
        try {
            searchModel.setCpyIdList(getCurrentLoginUser().getCpyIdList());
    ...
    @ResponseBody
    public void checkUserAccountIsUnique(Long cpyId, String userAccount) {
        JsonResult result = new JsonResult();
        UserDO loginUser = getCurrentLoginUser();

        try {
            boolean isUnique = userService.checkUserAccountIsUnique(cpyId, userAccount);
            result.setDataObject(isUnique);

            responseJson(result);
        } catch (Exception e) {
            LOGGER.error("UserController-checkUserAccountIsUnique is error|cpyId={}|userAccount={}|loginUser={}", cpyId, userAccount, loginUser, e);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/addUserCompanyBatch", method = RequestMethod.POST)
	@ResponseBody
	public void addUserActivity(UserDO user,@RequestParam(value = "file", required = false) MultipartFile file,HttpServletRequest request) {
		UserDO loginUser = getCurrentLoginUser();
		JsonResult result = new  JsonResult() ;
 		try {
			BaseResultDTO resultDTO = userService.addUserCompanyBatch(user,file,loginUser);
			if(resultDTO.isFailed()){
				result = new JsonResult(false,ResultCodeConstants.FAILED,resultDTO.getErrorDetail());
			}else {
				if (resultDTO.getErrorDetail()!=null) {
					result.setMsg(resultDTO.getErrorDetail());
				}
			}
			responseJson(result);
		} catch (Exception e) {
			LOGGER.error(this.getClass()+".addUserCompanyBatch() error:",e);
			responseJson(new JsonException("批量新增用户失败"));
			return;
		}
	}
    ...
    public void exportUser(HttpServletResponse response, UserDO searchModel) {
        ...
        try {
            ...
            userService.exportUser(response, searchModel, loginUser);
        } catch (Exception e) {
            LOGGER.error("UserController-exportUser is error|searchModel={}|loginUser={}", searchModel, loginUser, e);
            ErrorHtmlUtil.createErrorPage(response, "导出用户信息失败！");
        }
    }
     ...
     * 修改用户等级
     */
    @RequestMapping(value = "/modifyUserLevel", method = RequestMethod.POST)
    @ResponseBody
    public void modifyUserLevel(UserDO user) {
        JsonResult result = new JsonResult();
        UserDO loginUser = getCurrentLoginUser();

        try {
            BaseResultDTO resultDTO = userService.modifyUserLevel(user, loginUser);
            if (resultDTO.isFailed()) {
                result = new JsonResult(false, resultDTO.getResultCode(), resultDTO.getErrorDetail());
            }
            ...
            LOGGER.error("UserController-modifyUserLevel is error|modifyUser={}|loginUser={}", user, loginUser, e);
            responseJson(new JsonException("修改用户等级失败！"));
        }
    }
    ...
    public void modifyLoginPassword(Long userId, String password) {
        ...
        try {
            BaseResultDTO resultDTO = userService.modifyLoginPassword(userId, password, loginUser);
            if (resultDTO.isFailed()) {
                result = new JsonResult(false, resultDTO.getResultCode(), resultDTO.getErrorDetail());
            }
            ...
            LOGGER.error("UserController-modifyLoginPassword is error|modifyUserId={}|password={}|loginUser={}", userId, password, loginUser, e);
            responseJson(new JsonException("修改登陆密码失败，请刷新页面后再试！"));
        }
    }
    ...
    @ResponseBody
    public void resetLoginPassword(String userIds) {
        JsonResult result = new JsonResult();
        UserDO loginUser = getCurrentLoginUser();

        try {
            BaseResultDTO resultDTO = userService.resetLoginPassword(userIds, loginUser);
            if (resultDTO.isFailed()) {
                result = new JsonResult(false, resultDTO.getResultCode(), resultDTO.getErrorDetail());
            }
            ...
            LOGGER.error("UserController-resetLoginPassword is error|modifyUserId={}|loginUser={}", userIds, loginUser, e);
