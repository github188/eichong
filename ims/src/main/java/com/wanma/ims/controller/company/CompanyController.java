...
public class CompanyController extends BaseController {
	...
	public void getCompanyListPage(Pager pager, CompanyDO companyDO) {
		...
		try {
			companyDO.setCpyIdList(getCurrentLoginUser().getCpyIdList());
	...
	public void addCompany(CompanyDO companyDO) {
		...
		try {
			...
			companyDO.setCreator(loginUser.getUserAccount());
			companyDO.setCreatorId(loginUser.getUserId());
			BaseResultDTO dto = companyService.addCompany(companyDO,loginUser);
			if(!dto.isSuccess()){
	...
	public void exportCompany(CompanyDO companyDO) {
		try{
			...
			ErrorHtmlUtil.createErrorPage(response, "导出公司数据失败");
		}
	}
	 ...
	 * 公司-付费策略
	 */
	@RequestMapping(value = "/getCpyPayRule")
	@ResponseBody
	public void getCpyPayRule(Integer cpyNumber) {
		JsonResult result = new JsonResult();
		try {
			if(ObjectUtil.isEmpty(cpyNumber)){
				result.setSuccess(false);
				result.setMsg("公司标识不能为空");
				return;
			}
			ResultDTO<Map<String,Object>> dto = companyService.getCpyPayRule(cpyNumber);
			if(!dto.isSuccess()){
				responseJson(new JsonResult(false, ResultCodeConstants.FAILED, dto.getErrorDetail()));
				return;
			}
			...
			LOGGER.debug(this.getClass() + ".getCompanyByMaster() error : ", e);
			responseJson(new JsonException("卡绑公司-获取公司付费策略资金账户类型失败"));
			return;
		}
		responseJson(result);
