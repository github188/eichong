...
public class ReportPowerStationOrderController extends BaseController {
    ...
    @RequestMapping(value = "/statisticsCompanyAndPowerStationChargeOrder", method = RequestMethod.POST)
    @ResponseBody
    public void statisticsCompanyAndPowerStationChargeOrder(ReportOrderDO searchModel, Pager pager) {
        JsonResult result = new JsonResult();
        try {
            Long total = powerStationChargeStatisticsService.countOrderWithCompanyAndPowerStation(searchModel);
            if (total <= pager.getOffset()) {
                pager.setPageNo(1L);
            }
            pager.setTotal(total);
            searchModel.setPager(pager);
            List<ReportOrderDO> resultList = powerStationChargeStatisticsService.statisticsOrderWithCompanyAndPowerStation(searchModel);
            result.setPager(pager);
            result.setDataObject(resultList);
            responseJson(result);
        } catch (Exception e) {
            LOGGER.error(this.getClass() + "-statisticsChargeOrder error", e);
            responseJson(new JsonException("获取渠道分站点统计列表失败！"));
        }
    }

    @RequestMapping(value = "/exportStatisticsCompanyAndPowerStationChargeOrder", method = RequestMethod.GET)
    @ResponseBody
    public void exportStatisticsCompanyAndPowerStationChargeOrder(HttpServletResponse response, ReportOrderDO searchModel) {
        try {
            powerStationChargeStatisticsService.exportStatisticsCompanyAndPowerStationChargeOrder(response, searchModel);
        } catch (Exception e) {
            LOGGER.error(this.getClass() + "-statisticsChargeOrder error", e);
            responseJson(new JsonException("导出渠道分站点统计列表失败！"));
        }
    }
}
