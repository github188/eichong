...
function dt(){
    var that = $(".page-tabs-content").children(".active");
    that.remove()
    $('.J_iframe[data-id="' + that.data("id") + '"]').remove();
}
$('.J_tabCloseThis').on("click", dt)
function j() {
    g($(".J_menuTab.active"))
}

//$(".J_tabShowActive").on("click", j);
$("body").on("click",".J_tabShowActive", j);
function e() {
    if (!$(this).hasClass("active")) {
        var k = $(this).data("id");
        $(".J_mainContent .J_iframe").each(function () {
...
//新开一个tab页面
function newTab(clickClass,tabName){
    $("body").on("click", clickClass,function(){
        var o = $(this).attr("href"), m = $(this).data("index"), l = tabName;
        iframeController(o, m, l);
    });
}

//==========================
function iframeController(o, m, l) {
    var k = true;
    if (o == undefined || $.trim(o).length == 0) {
        return false
    }
