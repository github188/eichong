...
var menuId = getCurruntMap['渠道分站点统计'];
$(function(){
    ctrlMenu(menuId);
...
function statisticsListSearch() {
    toGiveHiddenInput();
    initTable("cpyStatisticsListForm", "cpyStatisticsListPage", statisticsCompanyAndPowerStationChargeOrderUrl, cpyStatisticsListCallback);
}
//下拉选项
toUnbindEvent();
...
function cpyStatisticsListCallback(req) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        listTr += '<tr>'
            ...
            + '</td><td class="statisticsList_payType">' + data[i].payType
    ...
    $("#myTbogy").html(listTr);
    var arr = ['statisticsList_date','statisticsList_company', 'statisticsList_station','statisticsList_payType',
...
$('#dataExport').on("click", function () {
    ...
    window.location.href = basePath + exportStatisticsCompanyAndPowerStationChargeOrderUrl + '?cpyNumber='
        + obj.cpyNumber + '&tagId='
