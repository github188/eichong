	...
	<form id="pagerForm" method="post"
		...
		<div class="searchBar">
					...
					<tr>
						...
						<td style="padding-left: 25px;"><span>消费类型&nbsp;</span> <select
							...
							style="align: left; width: 162px">
								...
								<option value="8"
									${params.puHi_Type == 8? 'selected="selected"' : ''}>退款</option>
								<%-- <option value="5"
						...
						<td id="areaSelecter"><span>区域选择</span> <select
							class="provinceCode required" id="moneyDetails_4SelProvince"
						...
						</select> <select class="cityCode required" id="moneyDetails_5SelCity"
							next="electricSelDistrict"
							data-val="${params.cityCode}" name="cityCode"
							style="float: none; width: 130px;">
								<option value="">--请选择城市--</option>
						</select> <select id="moneyDetails_6SelDistrict"
			...
			<table class="searchContent">
				...
				<tr style="color: red;">
					<td style="font-size: 20px; padding-left: 20px; line-height: 30px;">总冻结:${userTotalRecharge.chRe_FrozenAmt}元</td>
					<td style="font-size: 20px; padding-left: 20px; line-height: 30px;">余额:${userTotalRecharge.norm_account}元</td>
					...
					<c:forEach items="${moneyRecordListByUserId}"
						...
						<tr target="" rel="${moneyRecordByUserId.pk_PurchaseHistory }"
							...
							<td><c:if
								...
								<c:if test="${moneyRecordByUserId.puHi_Type == '8'}">
									<p>退款</p>
								</c:if></td>
