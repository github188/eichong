var pkMessageInfoId = getUrlParam('pkMessageInfoId');
//下拉列表
...
toLoadMessageInfo();
var powerStationArr = [];
function toLoadMessageInfo(){
    $.ajax({
        type: "post",
        url: basePath + getMessageInfoByIdUrl,
        async: true,
        data: {
            pkMessageInfoId: pkMessageInfoId
        },
        success: function (req) {
            var data = req.dataObject;
            var messageInfoType =data.messageInfoType;
            var messageInfoName = data.messageInfoName;
            var messageInfoContent = data.messageInfoContent;
            var messageInfoProvinceCode = data.messageInfoProvinceCode;
            var messageInfoCityCode = data.messageInfoCityCode;
            var messageInfoBegintime = data.messageInfoBegintime;
            var powerstationList = data.powerstationList;
            for(var i=0;i<powerstationList.length;i++){
                var stationName = powerstationList[i].mprName;
                var num = powerstationList[i].pkPowerstation;
                var obj ={
            ...
            if(messageInfoType==1){
                $('#messageInfoType').html('故障');
                $('#messageInfoType').attr('data-value',1);
            }else if(messageInfoType==2){
                $('#messageInfoType').html('新建');
                $('#messageInfoType').attr('data-value',2);
            }
            $('#messageInfoName').val(messageInfoName);
            $('#messageInfoContent').val(messageInfoContent);
            $('#messageInfoProvinceCode').attr('data-value',messageInfoProvinceCode);
            $('#messageInfoCityCode').attr('data-value',messageInfoCityCode);
            $('#messageInfoBegintime').val(messageInfoBegintime);
            setTimeout(function () {
                toLoadProvince(messageInfoProvinceCode, '#messageInfoProvinceCode', '.messageInfoProvinceCodeUl', 'toUnbindEvent');
            }, 200);
            setTimeout(function () {
                toLoadCity(messageInfoProvinceCode, messageInfoCityCode, '#messageInfoCityCode', '.messageInfoCityCodeUl', 'toUnbindEvent');
            }, 400);
        }
    });
}
...
function toSaveMessageInfo() {
    ...
    $.ajax({
        type: "post",
        url: basePath + updateMessageInfoUrl,
        async: true,
        data: obj,
        success: function (data) {
            if (data.success == true) {
