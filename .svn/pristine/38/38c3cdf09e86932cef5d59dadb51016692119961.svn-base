...
@Service
public class CmsCsjBespokeIncomeExtExcelServiceImpl extends CmsExcelServiceImpl {
	@Autowired
	private CmsExcelReportMapper excelReportMapper;
	private String[] columns = { "预约订单编号", "纯商家名称","电桩编号","枪头编号","预约时长(分钟)","预约单价(元)","冻结金额(元)","消费金额(元)","预约状态","开始时间","结束时间","实际预约结束时间" };//初始化列名
	...
	protected void makeExcelData(HSSFWorkbook wb, Object paramsModel) {
		...
		List<Map<String, Object>> list = excelReportMapper
				.queryCsjBespokeIncomeExtList((ExcelParamModel)paramsModel);
