...
public class ProcessInfoCommon {
	 ...
	 * @throws 无
	 */
	public static void setUpdateUserInfo(Entity commonModel,
...
//	}
	 ...
	 * @throws 无
	 */
	public static void setCreateUserInfo(Entity commonModel,
			HttpServletRequest request) {
	...
	public static void setDefaultPassword(TblUser TblUser) {
		 ...
		 * 系统配置文件中设置的默认密码 // String settingPassword = "";
		 * 
		 * // 系统配置文件信息 Map<String, String> propertiesMap = null;
		 * 
		 * // 取得系统配置文件信息 propertiesMap = MessageManager.getMessageManager()
		 * .getSystemProperties();
		 * 
		 * // 从系统配置文件中取得设置的默认密码 settingPassword = propertiesMap
		 * .get(CommonConsts.MAP_KEY_DEFAULT_USER_PASSWORD);
		 * 
		 * // 如果系统配置文件设置了默认密码，以系统设置的默认密码优先 if
		 * (StringUtil.isNotEmpty(settingPassword)) { defaultPassword =
		 * settingPassword; }
		 * 
		 * // 设置创建用户ID为登录用户Id //TblUser.setUserPassword(defaultPassword);
		 */}
	...
	public static void setDefaultPassword1(TblUser TblUser) {
		...
		// 从系统配置文件中取得设置的默认密码
		settingPassword = GlobalSystem
				.getConfig(CommonConsts.MAP_KEY_DEFAULT_USER_PASSWORD);

		// 如果系统配置文件设置了默认密码，以系统设置的默认密码优先
		if (StringUtils.isNotBlank(settingPassword)) {
	...
	public static void main(String args[]) {
		System.out.println(ProcessInfoCommon.getDigitRandomKey(4));
	}
}
