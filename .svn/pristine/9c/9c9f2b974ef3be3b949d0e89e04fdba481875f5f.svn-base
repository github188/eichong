...
<form id="pagerForm" method="post" action="order/FirmShop.do" onsubmit="return navTabSearch(this);"> 
...
<div class="pageContent">
	...
	<table class="table" width="100%" layoutH="86">
		...
		<tbody>
		<c:forEach items="${shopFirmList}" var="shopFirm" varStatus="status">
			<tr target="pkOrder" rel="${shopFirm.pkOrderdetail }" align="center">
				<td>
					<input name="pkorders"  value="${shopFirm.pkOrderdetail }"
						type="checkbox" />
				</td>
				<td>${ status.index + 1 + pager.numPerPage*(pager.pageNum-1)}</td>
				<td>${shopFirm.ordeProductname }</td>
				<td>${shopFirm.prodProductCode }</td>
				<td>${shopFirm.prodUser }</td>
				<td>${shopFirm.ordeTotalamount }</td>
				<td>${shopFirm.ordePrice }</td>
				<td>${shopFirm.ordeQuantity }</td>
				<td>
					<fmt:formatDate value="${shopFirm.ordeCreatedate }" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
		</c:forEach>
		</tbody>
	</table>
