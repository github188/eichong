...
public class MessageManager {
	...
	/** 接口配置文件 */
	private static Properties itfProperties = null;
	/** 数据库连接配置文件 */
	...
	static {
		...
		try {
			...
			// 接口配置文件
			InputStream inputStreamItf = MessageManager.class.getClassLoader()
					.getResourceAsStream(
							CommonConsts.PRO_FILE_INTERFACE_SETTING);
			...
			try {
				...
				// 接口配置文件
				itfProperties = new Properties();
				itfProperties.load(inputStreamItf);

				// 记录接口设置文件加载成功
				log.info("Secure " + CommonConsts.PRO_FILE_INTERFACE_SETTING
						+ " is loaded.");
	...
	public Map<String, String> getSystemProperties() {
		...
		// 用户默认密码
		propertiesMap
				.put(CommonConsts.MAP_KEY_DEFAULT_USER_CIPHER,
						sysProperties
								.getProperty(CommonConsts.PRO_KEY_DEFAULT_USER_CIPHER));
	...
	public String getInterfaceProperties(String messageKey) {
		// 信息内容
		String message = "";
		// 信息内容
		message = itfProperties.getProperty(messageKey);
		if (message == null) {
			// 将KEY设置成值
			message = messageKey;
		}
	...
	public static Map<String, String> getJdbcMap() {
		...
		// 密码
		propertiesMap.put(CommonConsts.dbCipher,
				jdbcProperties.getProperty(CommonConsts.PRO_KEY_DB_CIPHER));
