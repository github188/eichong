...
public class SessionMgr {
     ...
     * @return
     */
    public static Userinfo getWebUser(HttpServletRequest request) {
        return (Userinfo) request.getSession().getAttribute(WEB_SESSION_USER_KEY);
    }

    /**
     * 添加用户至session
     */
    public static void addWebUser(HttpServletRequest request, Userinfo user) {
        if(user==null) return;
        HttpSession session = request.getSession();
        session.setAttribute(WEB_SESSION_USER_KEY, user);
        session.setAttribute(WEB_SESSION_USER_pk, user.getPkUserId());
    }

    /**
     * 添加用户至session
     */
    public static void addWebUsers(HttpServletRequest request, TblUser user) {
        if(user==null) return;
        HttpSession session = request.getSession();
        session.setAttribute(WEB_SESSION_USER_KEY, user);
    ...
    public static String getUserId(HttpServletRequest request){
        return (String) request.getSession().getAttribute(WEB_SESSION_USER_pk);
    }
