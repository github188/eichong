...
import com.wanma.controller.VCmsChartController;
import com.wanma.model.echarts.EchartsParamModel;
import com.wanma.service.CmsEchartsService;

public abstract class CmsEchartsServiceImpl implements CmsEchartsService {
	protected static Logger log = Logger.getLogger(CmsEchartsServiceImpl.class);
	protected String path = "";

	public CmsEchartsServiceImpl() {
		try {
	...
	public String getJsonData(EchartsParamModel paramsModel) {
		log.info("******************图表JSON数据获取-begin************************");
		JSONObject obj = null;
		try {
			// 图表结构在前端生成，只需后端传输数据时,onlyData设置为1
			if ("1".equals(paramsModel.getOnlyData())) {
				obj = new JSONObject();
			} else {
				String path = VCmsChartController.class.getClassLoader()
						.getResource("").toURI().getPath();
				// 报表样式JSON格式
				obj = JsonUtil.getJsonObject(path + "/json/"
						+ paramsModel.getType() + ".json");
			}
			// 根据数据源设置属性值
			setData(obj, paramsModel);
		} catch (URISyntaxException e) {
			e.printStackTrace();
		}
