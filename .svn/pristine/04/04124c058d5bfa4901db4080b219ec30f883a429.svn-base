	...
	<div class="searchBar">
		...
		<table class="searchContent">
				<tr style="color: green;">
					<td style="font-size:20px;  padding-left: 20px;line-height: 30px;">总充值:<fmt:formatNumber value="${totalRecharge.AllPaypuHi_Monetary*5}" pattern="#.00"/>元</td>
					<td style="font-size:18px;">支付宝充值:<fmt:formatNumber value="${totalRecharge.aliPayPu_Monetary*5}" pattern="0.00"/>元</td>
					<td style="font-size:18px;">微信充值:<fmt:formatNumber value="${totalRecharge.weChatPu_Monetary*5}" pattern="0.00"/>元</td>
					<td style="font-size:18px;">平台人工充值:<fmt:formatNumber value="${totalRecharge.chargeCardPu_Monetary*5}" pattern="0.00"/>元</td>
					<td style="font-size:18px;">换卡转账:<fmt:formatNumber value="${totalRecharge.TransferAccount_Monetary*5}" pattern="0.00"/>元</td>
					<td style="font-size:18px;">7月活动送:<fmt:formatNumber value="${totalRecharge.activity_Monetary*5}" pattern="0.00"/>元</td>
					<!-- <td style="font-size:18px;">银联充值:${totalRecharge.unionpayPu_Monetary}</td> -->
				</tr>
				<tr style="color: red;">
					<td style="font-size:20px;  padding-left: 20px;line-height: 30px;">总消费:<fmt:formatNumber value="${totalPurchase.AllConsume*5}" pattern="#.00"/>元</td>
					<td style="font-size:18px;">充电消费:<fmt:formatNumber value="${totalPurchase.chargeConsume*5}" pattern="#.00"/>元</td>
					<td style="font-size:18px;">预约消费:<fmt:formatNumber value="${totalPurchase.orderConsume*5}" pattern="#.00"/>元</td>
					<!-- <td style="font-size:18px;">购物消费:${totalPurchase.shopConsume}</td> -->
				</tr>	
					<tr style="color: red;">
					<td style="font-size:20px;  padding-left: 20px;line-height: 30px;">总余额:<fmt:formatNumber value="${totalAccount.All_normal_account*5}" pattern="#.00"/>元</td>
	...
	<table class="table" width="100%" layoutH="190">
		...
		<thead>
			<tr align="center">					
			    ...
			    <th><bmtag:message messageKey="总优惠(元)" /></th> 
		...
		<c:forEach items="${userMoneyRecordList}" var="userMoneyRecord" varStatus="status">
			<tr target="puHi_UserId" rel="${userMoneyRecord.puHi_UserId }" align="center">			
				...
				<td><fmt:formatNumber value="${userMoneyRecord.userTotal_recharge*5}" pattern="0.00"/></td>
				<td><fmt:formatNumber value="${userMoneyRecord.userTotal_Purchase*5}" pattern="0.00"/></td>
				<td><fmt:formatNumber value="${userMoneyRecord.userTotal_Coupon*5}" pattern="0.00"/></td>
				<td><fmt:formatNumber value="${userMoneyRecord.norm_account*5}" pattern="0.00"/></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
