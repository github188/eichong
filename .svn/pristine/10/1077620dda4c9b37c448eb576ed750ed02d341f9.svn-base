 ...
 */
@Controller
@RequestMapping("/manage/config/level")
public class LevelController extends BaseController{
	 ...
	 * @return
	 */
	@RequestMapping(value = "/getLevelByCpyNum")
	@ResponseBody
	public void getLevelByCpyNum(Integer cpyNumber,HttpServletRequest request) {
		JsonResult result = new JsonResult();
		List<LevelDO> levelList = null;
		try {
			CompanyDO companyDO = new CompanyDO();
			companyDO.setCpyNumber(cpyNumber);
			List<CompanyDO> companyList = companyService.getCompanyList(companyDO);
			if (companyList.size()>1) {
				responseJson(new JsonResult(false,ResultCodeConstants.EXCEPTION,"公司标识"+cpyNumber+"错误"));
			}
			for (CompanyDO company : companyList) {
				levelList = levelService.getLevelByCpyId(company.getCpyId());
			}
			result.setDataObject(levelList);
		} catch (Exception e) {
			log.error(this.getClass()+".getLevelList() error:",e);
