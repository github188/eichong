var newsInfoId = getUrlParam('newsInfoId');
//加载信息
toLoadMessageInfo();
function toLoadMessageInfo(){
    $.ajax({
        type: "post",
        url: basePath + getNewsInfoByIdUrl,
        async: true,
        data: {
        ...
        success: function (req) {
            ...
            $('#picUrl').css('display','inline-block');
            $('.pic').attr('src',data.newsInfoPicUrl);
            $('#newsInfoName').val(data.newsInfoName);
            $('#newsInfoUrl').val(data.newsInfoUrl);
        }
    });
}
...
function toSaveNewsInfo(){
    ...
    formData.append('newsInfoId',newsInfoId);
    if(!fileName){
