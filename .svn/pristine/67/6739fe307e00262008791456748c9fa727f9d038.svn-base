...
function adminHomePileListCallback(req) {
    ...
    var listTr = "";
    for (var i = 0; i < data.length; i++) {
        listTr += '<tr><td><input type="checkbox" name="ids" class="selectedBox" value="' + data[i].electricPileId + '"/>'
...
function toAjaxPowserStation(cpyObject) {
    $.ajax({
        ...
        success: function (data) {
            if (data.success == true) {
                var dataModule = data.dataObject;
                var powerStationLi = '<li data-option="" class="seleced">请选择</li>';
                for (var i = 0; i < dataModule.length; i++) {
...
$("body").off("click", "#addPowerStation").on("click", "#addPowerStation", function () {
    ...
    layer.open({
        type: 1,
        offset: '100px',
        title: ["增加充电点", "font-size:12px;text-align:left;padding-left:20px;"],
        shadeClose: false,
        closeBtn: 1,
        area: ['310px', '330px'],//宽高
        content: $('.addPowerStation'),
        btn: ["确定", "取消"],
        yes: function (index, layero) {
            toCheckedPowerStation();
        },
        cancel: function (index, layero) {
            layer.closeAll();
        }
...
function toCheckedPowerStation(){
    $('.stationTip').html('');
    $('.stationTip').css('display','none');
    var powerStationId=$('input[name=powerStationId]').val();
    if(powerStationId==''){
        $('.stationTip').html('请输入充电点名称');
        $('.stationTip').css('display','block');
        return false;
    }
    toAddPowerStation();
    layer.closeAll();
}
function toAddPowerStation(){
    $.ajax({
        type: "post",
        url: basePath + addCountAdminEPByPsIdUrl,
        async: true,
        data: {
            userId: userId,
