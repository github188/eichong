...
@Service
public class CmsGtsjBespokeIncomeExtExcelServiceImpl extends CmsExcelServiceImpl {
	@Autowired
	private CmsExcelReportMapper excelReportMapper;
	private String[] columns = { "个体商家名称", "桩体编号","枪头编号","预约订单编号","预约时长(分钟)","预约单价(元/分钟)","消费金额(元)","续预状态","订单支付类型" };//初始化列名
	...
	protected void makeExcelData(HSSFWorkbook wb, Object paramsModel) {
		...
		List<Map<String, Object>> list = excelReportMapper
				.queryGtsjBespokeIncomeExtList((ExcelParamModel)paramsModel);
