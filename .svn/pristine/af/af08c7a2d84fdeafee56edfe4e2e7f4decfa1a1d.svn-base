...
public class CompanyManagerServiceImpl implements CompanyManagerService{
	...
	public void addCompany(TblCompany tblCompany,MultipartFile[] IdUnitCardImage,MultipartFile[] LicenseImage,
		...
		MultipartFileUtil.saveAllMulti(tblCompany,
				WanmaConstants.MULTI_UNITTYPE_ID_CARD_IMAGE, JudgeNullUtils.nvlStr(tblCompany.getPkCompanyid()));
		tblCompany.setAllMultiFile(LicenseImage);
		//营业执照
		MultipartFileUtil.saveAllMulti(tblCompany,
				WanmaConstants.MULTI_TYPE_LICENSE_IMAGE, JudgeNullUtils.nvlStr(tblCompany.getPkCompanyid()));
		tblCompany.setAllMultiFile(AffairsImage);
		//税务登记证
		MultipartFileUtil.saveAllMulti(tblCompany,
				WanmaConstants.MULTI_TYPE_AFFAIRS_IMAGE, JudgeNullUtils.nvlStr(tblCompany.getPkCompanyid()));	
		tblCompany.setAllMultiFile(AccreditImage);
		//授权证明
		MultipartFileUtil.saveAllMulti(tblCompany,
				WanmaConstants.MULTI_TYPE_ACCREDIT_IMAGE, JudgeNullUtils.nvlStr(tblCompany.getPkCompanyid()));	
	...
	public void modifyCompany(TblCompany tblCompany,MultipartFile[] IdUnitCardImage,MultipartFile[] LicenseImage,
			MultipartFile[] AffairsImage,MultipartFile[] AccreditImage) {
		// TODO Auto-generated method stub
		TblCompany tblCompany1=tblCompanyMapper.get(tblCompany.getPkCompanyid());
		...
		if (ObjectUtil.isNotEmpty(IdUnitCardImage)) { 
			...
			if (MultipartFileUtil.verifyImageIsNotNull(tblCompany)) {
				...
				List<String> allMultiFileName = multipartFileDao .getAllMultiFileName(
						WanmaConstants.MULTI_UNITTYPE_ID_CARD_IMAGE,tblCompany.getPkCompanyid() + "");
		       MultipartFileUtil.delelteMulti(allMultiFileName, WanmaConstants.MULTI_UNITTYPE_ID_CARD_IMAGE,
		...
		if (ObjectUtil.isNotEmpty(LicenseImage)) { 
			...
			if (MultipartFileUtil.verifyImageIsNotNull(tblCompany)) {
				MultipartFileDao multipartFileDao = new MultipartFileDao();
				List<String> allMultiFileName = multipartFileDao .getAllMultiFileName(
						WanmaConstants.MULTI_TYPE_LICENSE_IMAGE,tblCompany.getPkCompanyid() + "");
		       MultipartFileUtil.delelteMulti(allMultiFileName, WanmaConstants.MULTI_TYPE_LICENSE_IMAGE,
				       tblCompany.getPkCompanyid() + "");
		...
		if (ObjectUtil.isNotEmpty(AffairsImage)) { 
			...
			if (MultipartFileUtil.verifyImageIsNotNull(tblCompany)) {
				MultipartFileDao multipartFileDao = new MultipartFileDao();
				List<String> allMultiFileName = multipartFileDao .getAllMultiFileName(
						WanmaConstants.MULTI_TYPE_AFFAIRS_IMAGE,tblCompany.getPkCompanyid() + "");
		       MultipartFileUtil.delelteMulti(allMultiFileName, WanmaConstants.MULTI_TYPE_AFFAIRS_IMAGE,
				       tblCompany.getPkCompanyid() + "");
		...
		if (ObjectUtil.isNotEmpty(AccreditImage)) { 
			...
			if (MultipartFileUtil.verifyImageIsNotNull(tblCompany)) {
				MultipartFileDao multipartFileDao = new MultipartFileDao();
				List<String> allMultiFileName = multipartFileDao .getAllMultiFileName(
						WanmaConstants.MULTI_TYPE_ACCREDIT_IMAGE,tblCompany.getPkCompanyid() + "");
		       MultipartFileUtil.delelteMulti(allMultiFileName, WanmaConstants.MULTI_TYPE_ACCREDIT_IMAGE,
				       tblCompany.getPkCompanyid() + "");
...
//		}
	...
	public boolean checkCpyCompanyNumber(TblCompany tblCompany) {
		return tblCompanyMapper.findCompanyFlagListCountByCompanyNumber(tblCompany) == 0;
	}
