...
public class UserBlackWhiteController extends BaseController{
	...
	* @date 2017-7-5
	...
	public void getUserBlackWhiteList(@ModelAttribute("pager") Pager pager,
		...
		for (UserBlackWhiteDO userBlackWhiteDO : userBlackWhiteList) {
			userBlackWhiteDO.setTypeName(userBlackWhiteDO.getType().equals(0) ? "白名单" : "黑名单");
		}
	...
	* @date 2017-7-5
	...
	public void addUserBlackWhite(@ModelAttribute UserBlackWhiteBO userBlackWhiteBO, Model model, HttpServletRequest request) {
		...
		try {
			result = userBlackWhiteService.addUserBlackWhite(userBlackWhiteBO, user);
		} catch (Exception e) {
			responseJson(e);
		}
