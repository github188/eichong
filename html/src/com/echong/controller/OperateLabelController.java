...
public class OperateLabelController extends AbstractController {
    ...
    String operateLabel(String app_id, String sig, String info) {
        ...
        if (EChongWangType.ACTION_START.equals(electricLabel.getAction())) {
            logger.info("operateLabel open is begin ,model={}", JSON.toJSON(model));
            //对充电桩进行过滤--begin
            ...
            logger.info("operateLabel close is end ,model={}", JSON.toJSON(model));
        } else {
    ...
    @ResponseBody
    String queryOffLinePile(Integer stationID) {
        operateLabelService.queryOffLinePile(Integer.valueOf(CommonConsts.E_CHONG_ORG),stationID);
        return "success";
    }
}
